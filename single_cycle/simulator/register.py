...
class Register(object):
	...
	def getRegisterRptStr(self):
		"""Return the Register condition as str"""
		rptStr = ''
		for idx, val in self.rgstDict.items():
			if type(idx) == int and idx in self.rgstChangeList:
				rptStr += '${0}: {1}\n'.format(str(idx).zfill(2), val)
		if 'HI' in self.rgstChangeList:
			rptStr += '$HI: {}\n'.format(self.rgstDict['HI'])
		if 'LO'	in self.rgstChangeList:
