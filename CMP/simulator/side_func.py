...
def readBinFile(fileName):
	...
	bytecontent = [strcontent[i:i+2] for i in range(len(strcontent)) if i%2==0]
	# print(listcontent)	
	return listcontent, bytecontent

def hexToBin(hexList, length = 32):
	return list(map(lambda h: bin(int(h, 16))[2:].zfill(length), hexList))
